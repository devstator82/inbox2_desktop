            ...
            lock(this.SyncRoot){
                if(this.State == SIP_TransactionState.Completed){
                    ...
                    try{
                        ...
                        // Log
                        if(this.Stack.Logger != null){
                            this.Stack.Logger.AddText(this.ID,"Transaction [branch='" + this.ID + "';method='" + this.Method + "';IsServer=false] timer G(INVITE response(3xx - 6xx) retransmission) updated, will triger after " + m_pTimerG.Interval + ".");
                        }
                    }
                    catch(Exception x){
                        OnTransportError(x);
