...
namespace HtmlAgilityPack
{
	...
	public class HtmlWeb
	{
		...
		public HtmlDocument Load(string url, string method)
		{
			...
			else
			{
				...
				if (uri.Scheme == Uri.UriSchemeFile)
				{
					...
					doc.DetectEncodingAndLoad(url.Replace("file:///", String.Empty), _autoDetectEncoding);
				}
				else
				{
					throw new HtmlWebException("Unsupported uri scheme: '" + uri.Scheme + "'.");
		...
		private HttpStatusCode Get(Uri uri, string method, string path, HtmlDocument doc)
		{
			...
			uri = new Uri(HttpUtility.HtmlDecode(uri.ToString()));
			...
			if (PreRequest != null)
			{
				...
				//{
				//    Console.WriteLine("Cookie " + cookie.Name + "=" + cookie.Value + " path=" + cookie.Path + " domain=" + cookie.Domain);
				//}				
			}
