...
namespace Inbox2.Channels.Twitter
{
	public class TwitterWebRequest
	{
		...
		public static XDocument PerformRequest(Uri sourceUri, string consumerKey, string consumerSecret, string token, string tokenSecret, byte[] bytes)
		{
			HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(sourceUri);
			request.Method = "POST";
			request.ContentType = "application/x-www-form-urlencoded";
			request.UserAgent = "inbox2";
			request.Headers.Add("Authorization", BuildOAuthParams(sourceUri, consumerKey, consumerSecret, token, tokenSecret, null));
