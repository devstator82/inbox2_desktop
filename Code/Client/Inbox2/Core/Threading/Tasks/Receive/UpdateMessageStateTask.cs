...
namespace Inbox2.Core.Threading.Tasks.Receive
{
	public class UpdateMessageStateTask : BackgroundTask
	{
		...
		void ChangeLabels(ChannelMessageHeader header)
		{
			...
			if (message.PostLabels.Any())
			{
				// Check if this is an add or remove
				for (int i = 0; i < message.PostLabels.Count; i++)
				{
					var label = message.PostLabels[i];
					var label1 = label;
					...
					try
					{
						...
						else
						{
							...
							if (!String.IsNullOrEmpty(label1.MessageNumber))
							{
								...
								if (i == message.PostLabels.Count - 1 &&
